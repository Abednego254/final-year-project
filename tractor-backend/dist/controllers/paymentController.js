...
exports.verifyPayment = exports.getPaymentStatus = exports.mpesaCallback = exports.initiateStkPush = void 0;
...
const getMpesaToken = async () => {
    ...
    const response = await axios_1.default.get(`${baseUrl}/oauth/v1/generate?grant_type=client_credentials`, { headers: { Authorization: `Basic ${credentials}` } });
...
const initiateStkPush = async (req, res) => {
    ...
    try {
        ...
        const bookingResult = await (0, db_1.query)('SELECT * FROM bookings WHERE id = $1 AND farmer_id = $2', [booking_id, farmer_id]);
        ...
        await (0, db_1.query)(`INSERT INTO payments (booking_id, phone, amount, mpesa_checkout_request_id, status)
       VALUES ($1, $2, $3, $4, 'pending')
       ON CONFLICT (booking_id) DO UPDATE SET mpesa_checkout_request_id = $4, status = 'pending'`, [booking_id, phone, booking.price, checkoutRequestId]);
        res.json({
...
const handleSuccessfulPaymentLogic = async (booking_id, paidAmount) => {
    // Check if already paid to prevent double processing
    const checkB = await (0, db_1.query)(`SELECT status FROM bookings WHERE id = $1`, [booking_id]);
    if (checkB.rows.length > 0 && checkB.rows[0].status === 'paid')
        return;
    await (0, db_1.query)(`UPDATE bookings SET status = 'paid' WHERE id = $1`, [booking_id]);
    // Get tractor ID and current farmer ID to handle "First Pay, First Served" logic and calculate earnings
    const b = await (0, db_1.query)('SELECT tractor_id, farmer_id FROM bookings WHERE id = $1', [booking_id]);
    if (b.rows.length > 0) {
        const tractor_id = b.rows[0].tractor_id;
        const farmer_id = b.rows[0].farmer_id;
        // Get the operator ID (owner of the tractor)
        const tr = await (0, db_1.query)('SELECT owner_id FROM tractors WHERE id = $1', [tractor_id]);
        ...
        if (operator_id) {
            await (0, db_1.query)(`INSERT INTO earnings (booking_id, operator_id, total_amount, system_fee, first_half, second_half, first_half_paid, second_half_paid)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [booking_id, operator_id, paidAmount, systemFee, operatorHalf, operatorHalf, true, false]);
        ...
        await (0, db_1.query)(`UPDATE tractors SET status = 'busy' WHERE id = $1`, [tractor_id]);
        // Find other pending bookings for this tractor
        const otherPending = await (0, db_1.query)(`SELECT id, farmer_id FROM bookings WHERE tractor_id = $1 AND status IN ('pending', 'accepted') AND id != $2`, [tractor_id, booking_id]);
...
const mpesaCallback = async (req, res) => {
    try {
        ...
        if (ResultCode !== 0) {
            ...
            await (0, db_1.query)(`UPDATE payments SET status = 'failed' WHERE mpesa_checkout_request_id = $1`, [CheckoutRequestID]);
            res.sendStatus(200);
            return;
        }
        ...
        const paymentResult = await (0, db_1.query)(`UPDATE payments SET status = 'completed', mpesa_transaction_id = $1
       WHERE mpesa_checkout_request_id = $2 RETURNING booking_id, amount`, [mpesaTransactionId, CheckoutRequestID]);
...
const getPaymentStatus = async (req, res) => {
    ...
    try {
        const result = await (0, db_1.query)('SELECT status, mpesa_transaction_id, amount, created_at FROM payments WHERE booking_id = $1', [bookingId]);
...
const verifyPayment = async (req, res) => {
    const { bookingId } = req.params;
    try {
        const result = await (0, db_1.query)('SELECT * FROM payments WHERE booking_id = $1', [bookingId]);
        if (result.rows.length === 0) {
            res.status(404).json({ message: 'No payment record found for this booking.' });
            return;
        }
        ...
        const baseUrl = process.env.MPESA_ENVIRONMENT === 'production'
            ? 'https://api.safaricom.co.ke'
            : 'https://sandbox.safaricom.co.ke';
        const stkResponse = await axios_1.default.post(`${baseUrl}/mpesa/stkpushquery/v1/query`, {
            BusinessShortCode: shortcode,
            Password: password,
            Timestamp: timestamp,
        ...
        if (data.ResultCode === "0") {
            const paymentResult = await (0, db_1.query)(`UPDATE payments SET status = 'completed', mpesa_transaction_id = $1
                 WHERE mpesa_checkout_request_id = $2 RETURNING booking_id, amount`, ['VERIFIED_MANUALLY', checkoutRequestId]);
            if (paymentResult.rows.length > 0) {
                const { booking_id, amount } = paymentResult.rows[0];
                const paidAmount = parseFloat(amount);
                await handleSuccessfulPaymentLogic(booking_id, paidAmount);
